# R packages

- [allocation](https://github.com/andrewraim/allocation): Exact optimal
  allocation algorithms for stratified sampling.

- [COMMultReg](https://github.com/andrewraim/COMMultReg): Conway-Maxwell
  multinomial regression.

- [COMPoissonReg](https://cran.r-project.org/package=COMPoissonReg):
  Conway-Maxwell Poisson (COM-Poisson) regression.

- [DirectSampling](https://github.com/andrewraim/DirectSampling): Direct
  sampling with a step function. Also see
  [DirectSampling.jl](https://github.com/andrewraim/DirectSampling.jl) for
  a Julia implementation.

- [fntl](https://cran.r-project.org/package=fntl): Numerical Tools for Rcpp and
  Lambda Functions.

- [ldr](https://cran.r-project.org/package=ldr): Methods for likelihood-based
  dimension reduction in regression

- [ManifoldOptim](https://cran.r-project.org/package=ManifoldOptim): An R
  interface to the [ROPTLIB][roptlib-url] library for Riemannian manifold
  optimization.

- [MixLink](https://cran.r-project.org/package=mixlink): Mixture link regression.

- [OverdispersionModelsInR](http://github.com/andrewraim/OverdispersionModelsInR):
  Some tools to help with MLE computation, especially for non-standard
  likelihoods found in overdispersion models.

- [raim](https://github.com/andrewraim/raim): a collection of my utility
  functions.

- [sfreader](https://github.com/andrewraim/sfreader): An R Package to Read
  Census Summary Files. Currently focused on the 2010 SF2 summary file.
 
- [stcos](https://cran.r-project.org/package=stcos): Space-time change of
  support.

- [vws](https://cran.r-project.org/package=vws): Vertical Weighted Strips in R
  using C++.

[roptlib-url]: https://www.math.fsu.edu/~whuang2/Indices/index_ROPTLIB.html

# LaTeX templates

- [latex-resume](software/latex-resume.qmd): a template for resumes/CVs.

- [latex-business-card](software/latex-business-card.qmd): a template for
  business cards.

- [beamerthemeraim](https://github.com/andrewraim/beamerthemeraim): a
  custom Beamer theme.

- [formula-sheet](software/formula-sheet.qmd): a template for formula sheets.

- [sweave-examples](https://github.com/andrewraim/sweave-examples): templates
  for Latex slides and articles using Sweave via knitr.

Note: I am not a LaTeX programmer, so there may be room for improvement in
these templates.

# Utilities

- [md-util](https://github.com/andrewraim/md-util): some command line scripts
  and templates to facilitate compiling markdown to various formats (PDF, html,
  Beamer) using Pandoc.

- [worker](https://github.com/andrewraim/worker): a tool to help run repetitive
  computational studies such as statistical simulations.

