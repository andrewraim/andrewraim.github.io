---
title: "Vim as a Simple IDE without Plugins II"
date: 2026-01-09
categories:
  - linux
...
current working directory is within an R package, we the menu will allow us to
build & install, generate documentation, run a CRAN check, etc. That will take
place in a terminal buffer running in the same Vim session. This post is
specific to R, but the pattern might extend to other languages.

The complete code is given in the following [vimrc][vimrc], which also includes
the material from the [prequel](2026-01-03-vim-ide.qmd). As before, you can
copy or import the contents into your own [vimrc][h-vimrc] file to make use of
them.

The final result is demonstrated in @fig-demo. Here is what is being shown.
...
1. try to build the package documentation, but are informed that no R
  terminal is running.
1. Start an R terminal in Vim.
1. Build the package documentation and PDF manual using the menu.
...
whether we are in an R package, we check for the existence of a `DESCRIPTION`
file in the current directory; if found, that will be considered the root of
the project. Otherwise, we repeat the check in the parent directory `/path/to`.
We proceed this way until there are no more parents to check. This follows the
idea from the `find_package_root` function in the [desc][desc] R package.
...
Notice that an exception is thrown if we cannot find a `DESCRIPTION` file. The
`fnamemodify` function is used to obtain the parent directory of the current
path. If the argument `path` is the root, the result `newpath` will also be the
root, and we have nothing else to check.

# Menu Operations and R Functions

If we are in an R package, our menu will carry out selected actions via
corresponding calls to `devtools`. Those actions and function calls are shown
in @tbl-devtools.
...
function that gets call when something is selected in the menu; its job is to
take the selection and carry out the desired action. Both of these functions
are defined in an `RProjectMenu` function, which we can invoke to bring up the
menu.

The following snippet is an abridged version of `RProjectMenu`. The
`MenuHandler` function is somewhat long, so the contents have been summarized
here via comments.
...
In our implementation, we look for a terminal by name using the call
`bufnr('R')`. This name will be set if we start the terminal using the method
developed in the [previous](2026-01-03-vim-ide.qmd) post, or if we do either
`:term('R')` or `:vert term('R')`. All three of these options start a dedicated
terminal for R which will exit when the R session ends.
...
1. Enter terminal-normal mode using keystrokes `<c-\><c-n>` (`Ctrl-\` then
   `Ctrl-n`).
1. Type the command ':file R'" and hit `Enter` to run it.
...
[previous](2026-01-03-vim-ide.qmd) post, we set the `<leader>` key to space bar.
