...
prompt. Here I will show some customizations that I am enjoying, displayed
in @fig-result.

![Prompt after customizations.][term-result]{#fig-result} 

The complete script is [here][prompt]. To activate it in a Bash session, use the
...
Another useful piece of information would be the return code from the previous
command. But let's only display if is something other than zero.
...
Notice that `prompt_prefix` takes one argument which is a return code. We
should call this first to ensure that it is the return code from the last
command.
...
the return code in `$?`, it may interfere with `prompt_prefix`.
:::
...
[128] araim@localhost:~/.config/systemd/user$
❱ cd vimrc
araim@localhost:~/.../systemd/user/vimrc$ ➜vimrc:main
❱ git rev-parse --show-toplevel
/home/araim/.config/systemd/user/vimrc
araim@localhost:~/.../systemd/user/vimrc$ ➜vimrc:main
❱
```
...
if this is omitted, the terminal will be glitchy when editing commands and
traversing the command history. The site [colors.sh](https://colors.sh)
...
the previous command, to serve as a visual indicator. We can update the
`prompt_prefix` function to handle the colors as follows.
...
Remove the colors from the beginning of `PS1`.
...
![Prompt with colors based on status.][term-color-status]{#fig-colors} 
...
[term-color-status]: /downloads/2026-01-06-bash-prompt/term-color-status.svg
