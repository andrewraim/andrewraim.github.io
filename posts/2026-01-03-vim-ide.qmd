---
title: "Vim as a Simple IDE without Plugins"
date: 2026-01-03
categories:
  - linux
...
![A quick demonstration of Vim as an IDE.][demo]{#fig-demo}

A file with the all of the functions and keybindings is given
[here][vimrc]. The easiest way to use it is to paste the
...
This material was developed using version 9.1.1882 of Vim. The embedded
terminal that we will need was introduced in version 8.1.

If you would prefer more sophisticated IDE capabilities than DIYing, see
[vim-slime][vim-slime], [R.nvim][R.nvim] for R within [Neovim][neovim],
...
our content to the first terminal in the list.
...
The function `term_list()` returns an array of terminal buffer numbers. If
there are no terminal buffers, a notification to the user is printed with
`echom`.  Otherwise, the argument `content` is sent to the selected terminal
using `term_sendkeys`.

# Spawn a Terminal with a Console Program
...
::: {.callout-tip}
The strings `'r'`, `'julia'`, and `'python'` represent designated file types in
Vim. To display the file type for the currently open buffer, use the command
...
in case the console crashed or the exit was otherwise not intentional.
:::
...
after the line. The move is accomplished by pattern-matching for `\S`, with
option `W` to avoid wrapping back to the beginning when we have reached the
last line of non-whitespace.  The command `:nohl` is used to suppress
highlighting during pattern searching.
...
Here we send the entire selection - which already includes a newline at the end - to
the console, then move the cursor at the end of the selection using 
`<backtick>` then `>`.

The second case handles regular visual mode which is character-by-character.
Here the selection ends with a non-newline character, so we explicitly send a
newline at the end.
...
[demo]: /downloads/2026-01-03-vim-ide/demo.svg
[vimrc]: /downloads/2026-01-03-vim-ide/vimrc
